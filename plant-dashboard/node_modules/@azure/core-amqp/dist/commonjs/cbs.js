...
class CbsClient {
     ...
     * CBS endpoint - "$cbs"
     */
    endpoint = constants_js_1.Constants.cbsEndpoint;
    /**
     * CBS replyTo - The receiver link name that the service should reply to.
     */
    replyTo = `${constants_js_1.Constants.cbsReplyTo}-${(0, rhea_promise_1.generate_uuid)()}`;
    /**
     ...
     */
    cbsLock = `${constants_js_1.Constants.negotiateCbsKey}-${(0, rhea_promise_1.generate_uuid)()}`;
    /**
    ...
    async init(options = {}) {
        ...
        try {
            ...
            log_js_1.logger.warning("[%s] An error occurred while establishing the cbs links: %s", this.connection.id, `${translatedError?.name}: ${translatedError?.message}`);
